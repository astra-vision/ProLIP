...
class ImageNet():
    ...
    def __init__(self, root, preprocess):
        ...
        self.val = torchvision.datasets.ImageNet(self.image_dir, split='train', transform=test_preprocess)
    ...
    def generate_fewshot_dataset(self, num_shots):
        ...
        for label, items in split_by_label_dict.items():
            imgs= random.sample(items, num_shots*2)
            targets = [label for i in range(num_shots*2)]
            imgs_train = imgs_train + imgs[:num_shots]
            targets_train = targets_train + targets[:num_shots]
            imgs_val = imgs_val + imgs[num_shots:]
            targets_val = targets_val + targets[num_shots:]
            
        train = copy.deepcopy(self.train)
        val = copy.deepcopy(self.val)
        train.imgs = imgs_train
        train.targets = targets_train
        train.samples = imgs_train
        val.imgs = imgs_val
        val.targets = targets_val
        val.samples = imgs_val
