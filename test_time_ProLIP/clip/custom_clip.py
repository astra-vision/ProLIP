...
class ClipImageEncoder(nn.Module):
    def __init__(self, device, arch="ViT-L/14", image_resolution=224, n_class=1000):
        ...
        clip, embed_dim, _ = load(arch, device=device, download_root=None)
...
class PromptLearner(nn.Module):
    ...
    def reset_classnames(self, classnames, arch):
        ...
        clip, _, _ = load(arch, device=self.device, download_root=None)
...
class ClipTestTimeTuning(nn.Module):
    def __init__(self, device, classnames, batch_size, criterion='cosine', arch="ViT-L/14",
        ...
        clip, _, _ = load(arch, download_root=None)
