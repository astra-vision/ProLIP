...
class LinAdapterRN(nn.Module):
    def __init__(self, weight, bias):
        super(LinAdapterRN, self).__init__()
        ...
        ### Linear adapter initialized with the identity matrix
        self.linear2 = nn.Linear(output_dim, output_dim,bias=False)
        self.linear2.weight = nn.Parameter(torch.eye(output_dim).to('cuda').type(torch.float16))
        self.linear2.weight.requires_grad =  True

    def forward(self, x_before_proj):
        return self.linear2(self.linear(x_before_proj))

class LinAdapterViT(nn.Module):
    def __init__(self, vit_proj):
        super(LinAdapterViT, self).__init__()
    ...
    def forward(self,x_before_proj):
        ...
        x = self.linear2(x_before_proj @ self.vit_proj)
        return x


class RegLinearAdapter(FSCLIPmethod):
    ...
    def forward(self,
        ...
        if cfg['backbone'] == "RN50" or cfg['backbone'] == "RN101":
            ...
            initial_params = torch.eye(weight_init.shape[0]).type(torch.float16).to('cuda')

        elif cfg['backbone'] == "ViT-B/16" or cfg['backbone'] == "ViT-B/32":
            vit_proj = state_dict["visual.proj"]
            initial_params = torch.eye(vit_proj.shape[1]).type(torch.float16).to('cuda')
        ...
        if cfg['search_lr']:
            ...
            for init_lr in lr_list:
                for init_lambda in lambda_list:
                    proj = self.search_init_hp(mse,cfg,init_lr,init_lambda, model, state_dict, text_weights,train_x_before_list,train_labels,initial_params)
        ...
        ###reinitialize proj with pre-training weights

        if cfg['backbone'] == "RN50" or cfg['backbone'] == "RN101":
            proj = LinAdapterRN(weight_init,bias_init)
            
        elif cfg['backbone'] == "ViT-B/16" or cfg['backbone'] == "ViT-B/32":
            proj = LinAdapterViT(vit_proj)
        ...
        for train_idx in range(self.cfg['train_epoch']):
            ...
            mse_loss = mse(initial_params.view(-1),proj.linear2.weight.view(-1))
    ...
    def search_init_hp(self,mse, cfg, lr_v,lambda_v, model, state_dict, text_weights,train_x_before_list,train_labels,initial_params):
