...
from .utils import build_cache_model, cls_acc
from .utils import compute_image_features


class VisProjRNFrozen(nn.Module):
    def __init__(self, weight, bias):
        super(VisProjRNFrozen, self).__init__()
...
class ProLIP_Tip(FSCLIPmethod):
    '''
    ProLIP with TIP Adapter-F
    '''
    ...
    def forward(self,
        ...
        proj_frozen = VisProjRNFrozen(weight_init,bias_init)
        
        ### 
        cache_keys, cache_values = build_cache_model(self.cfg, model, train_loader,task,proj_frozen)
        ...
        params = list(proj.parameters()) + list(adapter.parameters())
        
        start_time = time.time()
        optimizer = torch.optim.AdamW(params, lr=lr_v, eps=1e-4)
        ...
        for train_idx in range(self.cfg['train_epoch']):
            ...
            image_features_frozen = proj_frozen(train_x_before)
            image_features_frozen = F.normalize(image_features_frozen,dim=-1)

            ### tip-adapter-F
            affinity = adapter(image_features_frozen)
        ...
        if cfg['dataset'] == 'imagenet':
            acc_test = self.compute_image_features_test(model, test_loader,adapter,text_weights,proj,proj_frozen,cache_values,alpha,beta)
            acc_test_v2 = self.compute_image_features_test(model, test_loader_v2,adapter,text_weights,proj,proj_frozen,cache_values,alpha,beta)
            acc_test_s = self.compute_image_features_test(model, test_loader_sketch,adapter,text_weights,proj,proj_frozen,cache_values,alpha,beta)
            acc_test_a = self.compute_image_features_test(model, test_loader_a,adapter,text_weights_a,proj,proj_frozen,cache_values[:,:200],alpha,beta)
            acc_test_r = self.compute_image_features_test(model, test_loader_r,adapter,text_weights_r,proj,proj_frozen,cache_values[:,:200],alpha,beta)
            ...
            test_features_frozen = proj_frozen(test_features_before)
            test_features_frozen = F.normalize(test_features_frozen,dim=-1)
            #####
            affinity = adapter(test_features_frozen)
            cache_logits = ((-1) * (beta - beta * affinity)).exp() @ cache_values
            clip_logits = 100. * test_features @ text_weights
            logits_test = clip_logits + cache_logits * alpha
    ...
    def compute_image_features_test(self,model, loader,adapter,text_weights,proj,proj_frozen,cache_values,alpha,beta):
        ...
        with torch.no_grad():
            for i, (images, target) in enumerate(loader):
                ...
                test_features_frozen = proj_frozen(x_before_proj)
                test_features_frozen = F.normalize(test_features_frozen,dim=-1)
