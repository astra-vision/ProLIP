...
class LASTLAYERFT(FSCLIPmethod):
    ...
    def forward(self,
        ...
        print('Turning off gradients for all layers except the last layer of the vision encoder')
        ...
        if cfg['backbone'] == "RN50":
            pretrained_params = model_init.visual.attnpool.state_dict()
        else:
            pretrained_params = model_init.visual.transformer.resblocks[-1].state_dict()
        ...
        for train_idx in range(self.cfg['train_epoch']):
            ...
            for i, (images, target) in enumerate(tqdm(train_loader)):
                ...
                mse_loss = 0.0
                if cfg['backbone'] == "RN50" or cfg['backbone'] == "RN101":
                    current_attnpool_params = model.visual.attnpool.state_dict()

                    for param_name, current_param in current_attnpool_params.items():
                        pretrained_param = pretrained_params[param_name]
                        loss = mse(current_param, pretrained_param)
                        mse_loss += loss

                elif cfg['backbone'] == "ViT-B/16" or cfg['backbone'] == "ViT-B/32":
                    current_params = model.visual.transformer.resblocks[-1].state_dict()
                ...
                loss_total = loss + lambda_v * mse_loss
